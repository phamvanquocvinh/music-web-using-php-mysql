        ...
        <div class="container">
            <div class="row">
                <div class="col-12 col-sm-12 col-md-8 col-lg-8">
                    ...
                    $sqlA = "SELECT playlistName FROM playlist WHERE playlistID = '$playlistID'";
                    $resultA = mysqli_query($conn, $sqlA);
                    while($rowA = $resultA->fetch_assoc()){
                        $playlistName = $rowA["playlistName"];
                        echo '<h2 style="background: linear-gradient(to right, #30CFD0 0%, #330867 100%);  background-clip: text;  color: white; border-radius: 10px">&nbsp&nbsp#'.$playlistName.'</h2>';
                        echo '<br><br>';
                    } 
                    ?>
            ...
            <div class="col-12 col-sm-12 col-md-4 col-lg-4">
                ...
                <div id="bxh">
                            <?php
                                ...
                                require_once("api/connect.php");
                                $playlistID = $_GET['p'];
                                $sqlA = "SELECT songID FROM playlist_song WHERE playlistID = '$playlistID'";
                                $resultA = mysqli_query($conn, $sqlA);

                                while($rowA = $resultA->fetch_assoc()){
                                    $songID = $rowA["songID"];
                                    $sqlB = "SELECT songName, artistID, songImg FROM song WHERE songID = '$songID'";
                                    $resultB = mysqli_query($conn, $sqlB);
                                    while($rowB = $resultB->fetch_assoc()){
                                        $songName = $rowB["songName"];
                                        $artistID = $rowB["artistID"];
                                        $songImg = $rowB["songImg"];
                                        $sqlC = "SELECT artistName FROM artist WHERE artistID = '$artistID'";
                                        $resultC = mysqli_query($conn, $sqlC);
                                        while($rowC = $resultC->fetch_assoc()){
                                            $artistName = $rowC["artistName"];

                                            echo '<div class="row g-0">';
                                                echo '<div class="col-1 col-sm-1 col-md-1 col-lg-1 rank1"><br>'.$count.'</div>';
                                                echo '<div class="col-2 col-sm-2 col-md-2 col-lg-2">';
                                                echo '<div class="card">';
                                                echo '<a href="playlistPlayer.php?s='.$songName.'&p='.$playlistID.'&o='.$count.'">';                                            
                                                    echo '<img src="'.$songImg.'" class="img-fluid" class="card-img-top">';                                                    
                                                echo '</div>';
                                                echo '</div>';
                                                echo '<div class="col-9 col-sm-9 col-md-9 col-lg-9" >';
                                                    echo '<div >';
                                                       echo '<h5 style="margin-left: 10px;">'.$songName.'</h5>';
                                                       echo '</a>';
                                                       echo '<a href="artist.php?s='.$artistName.'">';
                                                        echo '<p style="margin-left: 10px;">'.$artistName.'</p>';
                                                        echo '</a>';
                                                    echo '</div>';
